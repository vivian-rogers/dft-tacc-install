...
surface = plt.scatter(x,y,c='#111111')
...
printf "plotting $file and $file2\n"
python << END
import matplotlib.pyplot as plt
import numpy as np
import matplotlib as mpl
...
with open('$file2') as fp:
	...
	line = fp.readline()
		...
		line = fp.readline()
		columns = line.split()
		if(len(columns)>1 and float(columns[0]) > 0):
		 x.append(float(columns[0]) + 1/2)
               	 y.append(float(columns[1]))

surface = plt.scatter(x,y,c='#111111',s=4)
#kpts = [xmin,0.7071,1.5731,2.5731,3.0731,xmax]
#kpts = np.array([0,0.7071,1.5731,2.5731,3.0731,3.5731])
#kptnames = ['W','L','G','X','W','K']
#i = 0
...
printf "plotting $file and $file2 and $file3\n"
python << END
import matplotlib.pyplot as plt
import numpy as np
import matplotlib as mpl
...
with open('$file3') as fp:
	...
	line = fp.readline()
		...
		columns = line.split()
		if(len(columns)>1 and float(columns[0]) <= 0):
		 x.append((-1) * float(columns[0]) + 0.5)
               	 y.append(float(columns[1]))
